...
DCG において、物体の表面に画像（テクスチャ）を貼り付けることで、少ない頂点数でリアルな質感を表現する技術をテクスチャマッピングと呼びます。このプログラムでは、アルファチャンネル（透明度）を含んだ車のタイヤの画像データ（tire.raw）を読み込み、3D空間に配置された単純な直方体（箱）の各面に貼り合わせて描画しています。
